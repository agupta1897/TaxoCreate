...
class HearstPatterns(object):
    ...
    def __init__(self, spacy_model_name='en_core_web_sm', extended=False):
        """
        ...
        if spacy_model_name == "":
            self.__spacy_nlp = None
            self.contains_tokenizer = False
        else:
            self.__spacy_nlp = spacy.load(spacy_model_name)
            self.contains_tokenizer = True
    ...
    """
    def find_hyponyms(self, rawtext, needs_chunk=True):
        """ This is the main entry point for this code.
        ...
        all_hyponyms = []
        if needs_chunk:
            assert self.contains_tokenizer, "[ERROR] Tokenizer is unspecified"
            np_tagged_sentences = self.chunk(rawtext)
            for sentence in np_tagged_sentences:
                all_hyponyms.extend(self.find_hyponyms_in_sentence(sentence))
        else:
            all_hyponyms.extend(self.find_hyponyms_in_sentence(rawtext))

        return all_hyponyms

    def find_hyponyms_in_sentence(self, sentence):
        hyponyms = []
